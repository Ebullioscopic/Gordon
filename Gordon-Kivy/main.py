try:
    ...
    openai.api_key = "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"
...
class PlantIcon(MDIconButton):
    infected = BooleanProperty(True)
    icon = "spa"
    theme_icon_color = "Custom"
    id_val = NumericProperty()
    def __init__(self,**kwargs):
        super().__init__(**kwargs)
    ...
    def __init__(self,**kwargs):
        ...
        for i in range(self.rows):
            for j in range(self.cols):
                self.add_widget(PlantIcon(
                    infected = self.random_boolean(0.87),
                    id_val = i+j,
                ))

    def random_boolean(self,lower_concentration):
        if random() < lower_concentration:
            return False
        else:
            return True
class FarmHandApp(MDApp):
    ...
    def change_to_main_screen(self,mobile,passw):
        if str(mobile) == "9429199029" and str(passw) == "hari":
    ...
    def get_response_without_objects(self,child_id=None):
        ...
        self.chatview_id.add_widget(
            Command(text=value, size_hint_x=size, halign=halign, font_name="Tamil"))
        #else:
        #    self.chatview_id.add_widget(
        ...
        if self.dest_language == "en":
            self.chatview_id.add_widget(
                Response(text=reply, size_hint_x=.75, font_name="Tamil"))
